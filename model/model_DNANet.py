...
class VGG_CBAM_Block(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.relu2 = nn.ReLU()  # Create separate ReLU instance
...
class SpatialAttention(nn.Module):
    ...
    def execute(self, x):
        ...
        max_out = jt.max(x, dim=1, keepdims=True)  # Jittor's max returns values directly, not (values, indices)
...
class Res_CBAM_block(nn.Module):
    def __init__(self, in_channels, out_channels, stride = 1):
        ...
        self.bn2 = nn.BatchNorm2d(out_channels)
        self.relu2 = nn.ReLU()  # Create separate ReLU for final activation
